...
ggplot(data1, aes(y=Gain, x = Diet, fill=Diet)) + geom_boxplot()
...
data(data2)
data2
ggplot(data2, aes(y=Gain, x = Treatments, fill=Treatments )) + geom_boxplot()

modelo = ea1(data2, design = 2)
modelo$`Adjusted means`

##Mudar a coluna
#dados = data.frame(colocar n


####Exercicios ANOVA####
dados1 = read.csv("dados1ANOVA.csv")
dados2 = read.csv('dados2ANOVA.csv')
dados3 = read.csv('dados3ANOVA.csv')

##Ex 01 - DIC
head(dados1)
ggplot(dados1, aes(y = y, x = trat, fill = trat)) + geom_boxplot() 
croqui = design.crd(dados1$trat, 5)
modelo = ea1(dados1, design = 1)
modelo$`Analysis of variance`
#H0 : não existe efeito de tratamento
#H1 : Ao menos um tratamento tem efeito 
# p-valor = 0.001, < 0.05, H0 rejeitado 
modelo$Means
#Dado o resultado do método tukey, pode-se concluir que temos duas melhores misturas
# onde é viavel escolher a mistura e ou a mistura b, pois as duas são semelhantes

##Ex 02 - DBC
head(dados2)
#boxplot maquina
ggplot(dados2, aes(y = perda, x = maquina, fill = maquina)) + geom_boxplot()
#boxplot bico
ggplot(dados2, aes(y = perda, x = bico, fill = bico)) + geom_boxplot()
...
##Ex03
head(dados3)
ggplot(dados3, aes(y = tempo, x = reagente, fill = reagente)) + geom_boxplot() 
...
tudo2[,1:1000,2]=0 #zerar B,G colunas
imageShow(tudo2)
...
##Abrir a imagem
tudo=readImage("Trabalho/DesmatamentoOriginal.jpg")
imageShow(tudo)
dim(tudo) #linhas x colunas x matrix(R, G, 😎
1080 * 1920 # 2.073.600
...
tudo2[,1:1000,1]=0 #zerar B,G colunas
imageShow(tudo2)

##Abre a amostra desmatada
desmatado=readImage('Trabalho/DesmatamentoAmostra.jpg')
imageShow(desmatado)
dim(desmatado)
426*901 #383.826

##Abre a amostra boa 
boa=readImage('Trabalho/BoaAmostra.jpg')
imageShow(boa)
dim(boa)
#383.826

#Abre a amostra do que vai ser ignorado
ignora=readImage('Trabalho/Ignorar.jpg')
imageShow(ignora)
dim(ignora)
323*194 #62.662
...
c(boa[,,1])
length(c(boa[,,1]))
##Criado um vetor da parte vermelha, verde e azul de cada imagem
mboa=cbind(c(boa[,,1]),
             c(boa[,,2]),
             c(boa[,,3]))
mignora=cbind(c(ignora[,,1]),
             c(ignora[,,2]),
             c(ignora[,,3]))
mdesmatado=cbind(c(desmatado[,,1]),
            c(desmatado[,,2]),
            c(desmatado[,,3]))
sample(1:5,4)
...
mboa=mboa[sample(1:383826,10000),]
dim(mboa)
mignora=mignora[sample(1:62662,10000),]
mdesmatado=mdesmatado[sample(1:383826,5000),]
dim(mdesmatado)
...
##Junto todas as amostras em uma matriz
dados=rbind(cbind(mignora,0),cbind(mboa,1),cbind(mdesmatado,2))
head(dados)
colnames(dados)=c("R","G","B",'Y')
head(dados)
dim(dados)
...
tabela = table(pred)
tabela[3]/(tabela[2] + tabela[3])
#porcentagem de 41,18% de desmatamento na imagem analizada

varImpPlot(modelo,sort = T,main ="Importacia")
